 ...
 * 
 * Create WebOSGrid, that allows user can be group by, filter or more.
 * @constructor
 * @param {Object} config - the user configuration.
 */
iNet.iwebos.ui.common.grid.WebOSGrid = Ext.extend(Ext.grid.GridPanel,{
	/**
	...
	initComponent:function(){
		...
		this.sortInfo = this.sortInfo || {field: 'id', direction: 'ASC'};
		
		// set default sort.
		var __dsort = this.dsort || {field: 'id', direction: 'ASC'} ;
		...
		// set default sort.
		store.setDefaultSort(__dsort.field, __dsort.direction) ;
		delete this.dsort ;
		...
		iNet.iwebos.ui.common.grid.WebOSGrid.superclass.initComponent.call(this) ;
	},
	 ...
	 */
	handleSearchEvent : function(params){
		// create object.
    	var index = 0;
    	var buf = [] ;
