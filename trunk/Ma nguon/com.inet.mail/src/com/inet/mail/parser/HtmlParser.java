 ...
 * 
 * @date 10.12.2008
 * Based on HTML Parser of Tsmich.
 */
public abstract class HtmlParser {
	...
	// TODO (@author fdietz): Add more special entities - e.g. accenture chars
	// such as ?
	...
	public static String stripHtmlTags(String s) {
		...
		// remove header
		s = HEADER_REMOVAL_PATTERN.matcher(s).replaceAll(StringService.EMPTY_STRING);
		...
		s = STRIP_TAGS_PATTERN.matcher(s).replaceAll(StringService.EMPTY_STRING);
	 ...
	 * This includes transformation of special (language specific) chars such as
	 * the Danish ? ? ? ? ? ?.
	 * 
	 * @param s
	 *            input string
	 ...
	 *         parts (null on error)
	 * @author karlpeder, 20030623 (moved from
	 *         org.columba.mail.gui.message.util.DocumentParser)
	 */
	public static String restoreSpecialCharacters(Charset charset, String s) {
		...
		while (matcher.find()) {
			matcher.appendReplacement(result, charset.decode(
					ByteBuffer.wrap(new byte[] { (byte) Integer
							.parseInt(matcher.group(1)) })).toString());
		}
	...
	/**
	 * Get HTML character set.
	 * @param htmlSource String - the given HTML source.
	...
	public static Charset getHtmlCharset(String htmlSource) {
		...
		return Charset.forName(IOService.CHAR_SET);
	}
	 ...
	 * @author karlpeder, 20030623 (moved from
	 *         org.columba.mail.parser.text.BodyTextParser)
	 */
	 ...
	 * 
	 * TODO (@author fdietz): Add support for smilies and coloring of quoted
	 * text
	 * 
	 * @param text
	...
	public static String substituteEmailAddress(String s) {
		// due to bug CA-174 changed: return
		// EMAIL_PATTERN.matcher(s).replaceAll("<A HREF=\"mailto:$1\">$1</A>");
		...
		while (pos < length) {
			if (noLinkMatcher.find(pos)) {
				...
				if (withLinkMatcher.find(pos)) {
					...
					// fix that with looking for an open link in the same line
					// before
					// on the way from left to the current position of the email
					// at s1
					// find the last open link <a
					Matcher openLink = Pattern.compile("<a",
							Pattern.CASE_INSENSITIVE).matcher(s);
					Matcher closeLink = Pattern.compile("</a>",
							Pattern.CASE_INSENSITIVE).matcher(s);
					int linkPos = 0;
					int savedLinkPos = -1;
					while (linkPos < s1) {
					...
					if (savedLinkPos > -1) {
						...
						if (closeLink.find(savedLinkPos)) {
							// if the closing mark is after the s1 mark do not
							// insert a link
							if (closeLink.end() >= s1) {
	...
	public static String substituteURL(String s) {
		...
		while (m.find()) {
			...
			if (match.matches(".*<\\w+$") && s.length() > m.end()
					&& s.charAt(m.end()) == '>') {
	...
	public static String removeComments(String html) {
		// remove comments
		return COMMENTS_REMOVAL_PATTERN.matcher(html).replaceAll(StringService.EMPTY_STRING);
	}
}
