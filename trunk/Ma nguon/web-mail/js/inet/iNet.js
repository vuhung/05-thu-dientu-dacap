...
Ext.namespace("iNet", 
			  ...
			  "iNet.iwebos.ui.calendar",
			  "iNet.iwebos.ui.admin",
			  "iNet.iwebos.ui.common.form.HtmlEditorPlugins"
	) ;
...
Ext.namespace(
		...
		"iwebos.message.paperwork.report",
		"iwebos.message.paperwork.ed.search",
		"iwebos.message.mail.contact",
		"iwebos.message.admin",
		"iwebos.message.calendar"
	);
...
(function(){
	...
	Ext.apply(Ext, {
        isGecko2: Ext.isGecko && check(/rv:1\.8/),
		isGecko4: Ext.isGecko && check(/rv:2\.0/)
	});
})();
...
Ext.apply(iNet, {
	 ...
	 * The ratify data.
	 */
	INET_RATIFY_DATA: null,
	/**
	 * The filter data.
	 */
	INET_FILTER_DATA: null,
	...
	dateGroupRenderer: function(){
		...
		var currentYearDays = Date.parseDate(year + "/01/01","Y/m/d").getTime() ;
		var lastYearDays = Date.parseDate(year + "/01/01","Y/m/d").add(Date.YEAR, -1).getTime() ;
...
Ext.onReady(function(){
	 ...
	 * copy type
	 */
	iNet.INET_COPY_TYPE = [[0,iwebos.message.paperwork.dc.copy_card_type_cf],[1,iwebos.message.paperwork.dc.copy_card_type_cp],[2,iwebos.message.paperwork.dc.copy_card_type_ci]] ;
	
	/**
	 * ratify data.
	 */
	iNet.INET_RATIFY_DATA = [[0,iwebos.message.dialog.ratify_return], [1,iwebos.message.dialog.ratify_select_user], [2,iwebos.message.dialog.ratify_submit]] ;
	/**
	 * filter data
	 */
	iNet.INET_FILTER_DATA=[[0,iwebos.message.dialog.filter_all],[1,iwebos.message.dialog.filter_group],[2,iwebos.message.dialog.filter_position],[3,iwebos.message.dialog.filter_leader],[4,iwebos.message.dialog.filter_document]];
});
